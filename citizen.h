...
// Citizen schema
typedef struct {
    char cpf[12];             // CPF (11 digits + '\0'), primary key
    char voterNumber[14];     // Voter number (14 digits + '\0') secondary key
    char name[100];           // Full name
    char phone[20];           // Phone number (including area code)
    char address[200];        // Full address
    char birthdate[11];       // Birth date in "YYYY-MM-DD" format
    int deleted;              // Logical deletion flag (0 = active, 1 = deleted)
} citizen;
...
void printDefaultCitizenBorder(); // Print the default border for showing citizens
void printShowCitizenHeader();  // Print the header for showing citizens
